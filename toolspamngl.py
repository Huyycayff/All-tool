...
class zNGL:
    ...
    def __init__(self,
        ...
        if os.name == "nt":
            os.system(
                f"title ЁЯТ░ {self.NAME_TOOL} {self.VERSION_TOOL} by telegram @wus_team ЁЯТ░"
            )
    ...
    def _input(self):
        while True:
            _user = Write.Input(
                "[+] Nhс║нp Link NGL hoс║╖c Username > ",
                Colors.red_to_yellow,
                interval=0.005,
            ).strip()
            if not _user:
                print(
                    f"{Fore.RED}тЪая╕П Vui l├▓ng nhс║нp Username hoс║╖c Link cс║зn spam.{Style.RESET_ALL}"
                )
                continue
            self._username = self._convert(_user)
            if not self._username:
                print(
                    f"{Fore.RED}тЪая╕П Vui l├▓ng nhс║нp username hoс║╖c URL (eg: https://ngl.link/username){Style.RESET_ALL}"
                )
                continue
            if not self._check_user(self._username):
                print(
                    f"{Fore.RED}тЪая╕П Ng╞░с╗Эi d├╣ng '{self._username}' kh├┤ng tс╗Уn tс║бi.{Style.RESET_ALL}"
                )
                continue
            break
        while True:
            try:
                self._threads = int(
                    Write.Input(
                        "[+] Nhс║нp Threads (1 - 500) > ",
                        Colors.red_to_yellow,
                        interval=0.005,
                    ).strip())
                if self._threads < 1 or self._threads > 500:
                    print(
                        f"{Fore.RED}тЪая╕П Sс╗С Threads Tс╗Сi Thiс╗Гu L├а 1, Tс╗Сi ─Рa L├а 500, Sс╗С Threads C├аng Cao {self.NAME_TOOL} Request C├аng Nhiс╗Бu.{Style.RESET_ALL}"
                    )
                    continue
                break
            except ValueError:
                print(f"{Fore.RED}тЪая╕П Threads Phс║гi L├а Sс╗С.{Style.RESET_ALL}")
        while True:
            self._question = Write.Input(
                "[+] Nhс║нp Th├┤ng ─Рiс╗Зp (C├│ Thс╗Г ─Рс╗Г Trс╗Сng) > ",
                Colors.red_to_yellow,
                interval=0.005,
            ).strip()
            if self._question == "":
            ...
            if len(self._question) > 70:
                print(
                    f"{Fore.RED}тЪая╕П Th├┤ng ─Рiс╗Зp Kh├┤ng ─Р╞░с╗гc Qu├б D├аi (Tс╗Сi ─Рa 70 K├╜ Tс╗▒).{Style.RESET_ALL}"
                )
                continue
            break
        if (Write.Input(
                "[?] K├нch Hoс║бt Random Emoji (yes/no) > ",
                Colors.red_to_yellow,
                interval=0.005,
        ).strip().lower() == "yes"):
...
┬й COPYRIGHT BY HUY CAYF TOOL 2025 
...
"""
    ...
    def zNGL(self, thread_id: int):
        icon = (random.choice([
            " ЁЯШК", " ЁЯШО", " ЁЯШН", " ЁЯШЙ", " ЁЯШБ", " ЁЯШД", " ЁЯШГ", " ЁЯЩВ", " ЁЯШЖ", " ЁЯШЕ", " ЁЯдг",
            " ЁЯШВ", " ЁЯШЛ", " ЁЯШЫ", " ЁЯШЬ", " ЁЯдк", " ЁЯдй", " ЁЯе░", " ЁЯШЗ", " ЁЯЩГ", " ЁЯе╣", " ЁЯШМ",
            " ЁЯдЧ", " ЁЯШП", " ЁЯдн", " ЁЯлв", " ЁЯла", " ЁЯдл", " ЁЯШн", " ЁЯШв", " ЁЯШе", " ЁЯШУ", " ЁЯШЮ",
            " ЁЯШФ", " ЁЯЩБ", " тШ╣я╕П", " ЁЯШа", " ЁЯШб", " ЁЯдм", " ЁЯШд", " ЁЯШЦ", " ЁЯШл", " ЁЯШй", " ЁЯе║",
            " ЁЯШ▒", " ЁЯШи", " ЁЯШ░", " ЁЯШ╡", " ЁЯдп", " ЁЯШ│", " ЁЯШм", " ЁЯлг", " ЁЯе┤", " ЁЯдв", " ЁЯдо",
            " ЁЯШ╖", " ЁЯдТ", " ЁЯдХ", " ЁЯдз", " ЁЯе╢", " ЁЯе╡", " ЁЯШИ", " ЁЯС┐", " ЁЯТА", " ЁЯС╗", " ЁЯС╜",
            " ЁЯШ║", " ЁЯШ╕", " ЁЯШ╣", " ЁЯШ╗", " ЁЯШ╝", " ЁЯШ╜", " ЁЯЩА", " ЁЯШ┐", " ЁЯШ╛", " ЁЯдб", " тЭдя╕П",
            " ЁЯзб", " ЁЯТЫ", " ЁЯТЪ", " ЁЯТЩ", " ЁЯТЬ", " ЁЯдО", " ЁЯЦд", " ЁЯдН", " ЁЯТУ", " ЁЯТЧ", " ЁЯТЦ",
            " ЁЯТШ", " ЁЯТЭ", " ЁЯТЮ", " ЁЯТХ"
        ...
        try:
            ...
            self.print_queue.put((
                f"{Fore.MAGENTA}ID-{self._random_num(5)} {Fore.GREEN}тАв{Style.RESET_ALL} "
    ...
    def run(self,
        ...
        print("")
        print("".join([
            f"{[Fore.MAGENTA, Fore.LIGHTMAGENTA_EX, Fore.LIGHTBLUE_EX, Fore.CYAN, Fore.LIGHTGREEN_EX, Fore.GREEN][i * 6 // len('┬╗┬╗тАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФ┬л┬л')]}{c}"
            for i, c in enumerate(
                "┬╗┬╗тАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФтАФ┬л┬л")
        ]) + Style.RESET_ALL)
        ...
        try:
            while True:
                ...
                if time.time() - start_time > self._timeout:
                    print(
                        f"{Fore.RED}тЪая╕П Thread {cycle_count} failed, trying to the next thread.{Style.RESET_ALL}"
                    )
            print(
